...
func (d *podmanProviderState) getClient(ctx context.Context, host string) (*client.Client, error) {
	...
	if host == "" {
		if d.DefaultHost == "" {
			return nil, fmt.Errorf(
				"resource must specify container_host if neither a default container_host is specified " +
					"in the provider nor a CONTAINER_HOST environment variable is set")
		}
	...
	c, err := client.Connect(ctx, url, &config)
