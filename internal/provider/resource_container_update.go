...
// parameter returns a slice of pointers to model object models, and this array has the same size
// as the `after` List. Elements in this array are non-nil if the non-write-only attributes of the
// element have changed, otherwise the entry is nil.
//
// In the case where there are no changes whatsoever then an empty slice is returned instead.
//
